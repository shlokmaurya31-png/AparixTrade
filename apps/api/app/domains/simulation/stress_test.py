...
A shock target is either a benchmark ("NIFTY50"), a sector name, or a single
holding's symbol. Benchmark shocks propagate through each holding's beta;
sector/symbol shocks apply directly to the matching holdings and zero to the
rest — that's a deliberate simplification (no cross-sector contagion model).
"""
...
def apply_shock(
    rows: list[HoldingRow], *, target: str, shock_pct: float, beta_by_symbol: dict[str, float]
) -> dict:
    ...
    return {
        ...
        "assumptions": (
            ...
            "scaled per holding by its historical beta vs NIFTY 50. No cross-sector contagion, "
            "liquidity, or second-order macro effects are modeled."
        ),
        "is_mock": True,
    }
